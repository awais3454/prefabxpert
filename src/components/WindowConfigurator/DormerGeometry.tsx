...
export function ClosedPanel({ W, H, frameColor, panelColor }: { W: number; H: number; frameColor: string; panelColor: string }) {
  ...
  // Flat, borderless panel — flush with the wall, same depth/position as the
  // other wall strips (FRONT_T), so a "Gesloten paneel" kozijn just looks
  // like a continuous flat wall surface with no frame/border around it.
  return (
...
/** Kader boei band on a side cheek — colors ONLY the front-facing (+Z) and the
 *  outward-facing side (±X, away from the window) with fasciaColor. The other
 *  faces (top, bottom, back, inward) stay on frontColor to match the
 *  adjoining wall, instead of the whole box being one uniform boei color. */
function BoeiSideBand({ position, size, mainColor, boeiColor, outward }: {
  ...
  // BoxGeometry face order: 0=+X, 1=-X, 2=+Y(top), 3=-Y(bottom), 4=+Z(front), 5=-Z(back)
  const m = mainMatRef.current
  const b = boeiMatRef.current
  const mats = outward === 'right'
    ? [b, m, m, m, b, m]   // +X (outward) + front = boei
    : [m, b, m, m, b, m]   // -X (outward) + front = boei
...
export function RoofSurface({ W, H, depth, pitchDeg, sideExt, isKader, roofTileColor }: { W: number; H: number; depth: number; pitchDeg: number; sideExt?: number; isKader?: boolean; roofTileColor?: string }) {
  ...
  const tileMat = useMemo(() => {
    ...
    const mat = new THREE.MeshStandardMaterial({
      ...
      // displacementMap removed — it was distorting the mesh at the irregular
      // hole-cut boundary around the dormer, producing a speckled/noisy
      // artifact along the roof edges next to the dormer sides.
      roughness:    0.7,
      metalness:    0.4,
...
export function ProceduralDormer({ config }: { config: WindowConfig }) {
  ...
  return (
    <group position={[0, -H / 2, 0]}>
      ...
      {/* ── Dark membrane plane on top of roof slab — sits flush with the
          slab's top surface (no floating gap), so no step is visible from
          above where it meets the boei bands on the sides ── */}
      {(() => {
        ...
        const slabLen = depth + ROOF_OVERHANG
        const slabCtrZ = (ROOF_OVERHANG - depth) / 2
        // Flush with the TOP of the fascia board (H+ROOF_SLAB_T+40mm) — sitting
        // here instead of inside the fascia board's own height range keeps it
        // visible instead of being hidden inside that solid box.
        // Sit clearly ABOVE the fascia top (not flush/coplanar — that caused
        // z-fighting/flicker — and not embedded inside it, which hid the
        // whole plate). A small gap above the fascia is fine since this is
      ...
      <group position={[halfW + (styleType === 'kader' ? mm(150) : 0), 0, styleType === 'kader' ? -mm(10) : 0]}>
        ...
        {/* Kader: boei band on the outer RIGHT face — same height as the front
            boei (slab + fascia board) so the colored line runs fully around.
            Only the front + outward (+X) faces get fasciaColor. */}
        {styleType === 'kader' && (
          <BoeiSideBand
      ...
      <group position={[-halfW - (styleType === 'kader' ? mm(150) : 0), 0, styleType === 'kader' ? -mm(10) : 0]}>
        ...
        {/* Kader: boei band on the outer LEFT face — same height as the front
            boei (slab + fascia board) so the colored line runs fully around.
            Only the front + outward (-X) faces get fasciaColor. */}
        {styleType === 'kader' && (
          <BoeiSideBand
